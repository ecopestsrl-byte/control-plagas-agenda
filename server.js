...
async function handleApi(req, res) {
  ...
  if (url.pathname === "/api/admin/users" && req.method === "GET") {
    if (user.role !== "admin") return send(res, 403, { ok: false, error: "Solo el dueno puede administrar usuarios" });
    const users = ensureUsers().map((item) => ({ username: item.username, role: item.role || "usuario" }));
  ...
  if (url.pathname === "/api/admin/reset-user" && req.method === "POST") {
    if (user.role !== "admin") return send(res, 403, { ok: false, error: "Solo el dueno puede administrar usuarios" });
    const users = ensureUsers();
    const target = users.find((item) => item.username === body.currentUsername);
    if (!target) return send(res, 404, { ok: false, error: "Usuario no encontrado" });
    const newUsername = String(body.newUsername || target.username).trim() || target.username;
