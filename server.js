...
function ensureUsers() {
  ...
  if (users && users.length) {
    ...
    if (!users.some((item) => item.role === "owner" || item.role === "admin")) {
      users[0].role = "owner";
    }
  ...
  const initial = [
    ...
    { username: "admin", passwordHash: hash("admin123"), role: "user" }
  ];
...
async function handleApi(req, res) {
  ...
  if (url.pathname === "/api/system-config" && req.method === "POST") {
    if (!isOwner(user)) return send(res, 403, { ok: false, error: "Solo el propietario puede modificar la configuracion" });
