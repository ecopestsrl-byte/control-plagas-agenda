...
function publicConfig(username = "") {
  const saved = normalizeConfig(readJson(SYSTEM_CONFIG_FILE, {}));
  const userStore = username ? readJson(userStorageFile(username), {}) : {};
...
async function handleApi(req, res) {
  ...
  if (url.pathname === "/api/public-config" && req.method === "GET") {
    return send(res, 200, { ok: true, config: publicConfig(url.searchParams.get("username") || "") });
  }

  if (url.pathname === "/api/login" && req.method === "POST") {
    const users = ensureUsers();
    const user = users.find((item) => item.username === body.username);
    const config = publicConfig(body.username);
