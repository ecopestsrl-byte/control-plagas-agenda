...
let state = { technicians: [], services: [], recurring: [], reports: {}, sentTechs: {}, config: { name: "CP Control Plagas", color: "#0f6b4f", supervisor: "", logo: "", loginInitials: "CP", loginTitle: "Control Plagas", companyCode: "CP" } };
...
function applyConfig() {
  state.config = { loginInitials: "CP", loginTitle: "Control Plagas", companyCode: "CP", ...(state.config || {}) };
...
$("credentialsForm").onsubmit = async (e) => {
  ...
  toast(data.ok ? "Usuario y contrasena guardados." : data.error || "No se pudo guardar.");
};
