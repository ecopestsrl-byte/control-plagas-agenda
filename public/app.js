...
async function load() {
  ...
  $("userBadge").textContent = `${currentUser.username || ""}${isOwner() ? " · propietario" : currentUser.role === "admin" ? " · admin" : ""}`;
  applyRoleAccess();
  const stored = await api("/api/storage");
  const publicData = await api(`/api/public-config?username=${encodeURIComponent(currentUser.username || "")}`);
  state = { ...state, ...stored };
  state.config = { ...(state.config || {}), ...(publicData.config || {}), ...(stored.config || {}) };
...
function applyConfig() {
  ...
  $("logoFallback").textContent = state.config.loginInitials || "CP";
...
document.querySelectorAll(".nav button[data-view]").forEach(btn => btn.addEventListener("click", () => {
  ...
  $("pageTitle").textContent = title === "Agenda" ? "Agenda y asignacion de servicios" : title;
  renderAll();
}));
...
$("configForm").onsubmit = async (e) => {
  ...
  await save();
  toast("Configuracion guardada para este usuario.");
};
