...
async function loadAdminUsers() {
  ...
  $("adminUsersList").innerHTML = data.users.map(u => `<div class="item"><div class="item-head"><strong>${u.username}</strong><span class="pill">${u.role}</span></div></div>`).join("");
  $("adminCurrentUser").innerHTML = data.users.map(u => `<option value="${u.username}">${u.username}</option>`).join("");
}
...
$("adminUserForm").onsubmit = async (e) => {
  ...
  if (data.ok) {
    $("adminUserForm").reset();
    toast("Usuario actualizado por el dueno.");
    loadAdminUsers();
  } else {
