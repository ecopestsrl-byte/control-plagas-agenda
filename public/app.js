...
async function load() {
  ...
  $("userBadge").textContent = `${currentUser.username || ""}${isOwner() ? " · dueno" : ""}`;
  applyRoleAccess();
...
function applyRoleAccess() {
  ...
  if (configBtn) configBtn.classList.toggle("hidden", !isOwner());
}
...
document.querySelectorAll(".nav button[data-view]").forEach(btn => btn.addEventListener("click", () => {
  if (btn.dataset.view === "config" && !isOwner()) {
    toast("Solo el dueno puede entrar a Configuracion.");
    return;
  }
...
async function loadAdminUsers() {
  ...
  $("adminUsersList").innerHTML = data.users.map(u => `<div class="item"><div class="item-head"><strong>${u.username}</strong><span class="pill">${u.role === "owner" || u.role === "admin" ? "dueno" : "usuario"}</span></div></div>`).join("");
