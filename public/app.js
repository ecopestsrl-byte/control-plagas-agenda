...
let state = { technicians: [], services: [], recurring: [], reports: {}, sentTechs: {}, config: { name: "CP Control Plagas", color: "#0f6b4f", menuColor: "#102820", supervisor: "", logo: "", loginInitials: "CP", loginTitle: "Control Plagas", companyCode: "CP", loginBackground: "#e9e9e9", loginBrandColor: "#e2261c", loginButtonColor: "#e2261c" } };
...
async function load() {
  ...
  currentUser = me.user || { username: "", role: "user" };
  $("userBadge").textContent = `${currentUser.username || ""}${isOwner() ? " · propietario" : canConfigureSystem() ? " · admin" : ""}`;
  applyRoleAccess();
  const stored = await api("/api/storage");
  const publicData = await api("/api/public-config");
  state = { ...state, ...stored };
  state.config = { ...(state.config || {}), ...(stored.config || {}), ...(publicData.config || {}) };
...
function applyRoleAccess() {
  ...
  document.querySelectorAll(".system-config").forEach((item) => item.classList.toggle("hidden", !canConfigureSystem()));
}
...
function applyConfig() {
  state.config = { menuColor: "#102820", loginInitials: "CP", loginTitle: "Control Plagas", companyCode: "CP", loginBackground: "#e9e9e9", loginBrandColor: "#e2261c", loginButtonColor: "#e2261c", ...(state.config || {}) };
  document.documentElement.style.setProperty("--brand", state.config.color || "#0f6b4f");
  document.documentElement.style.setProperty("--menu", state.config.menuColor || "#102820");
  ...
  $("cfgMenuColor").value = state.config.menuColor || "#102820";
...
$("configForm").onsubmit = async (e) => {
  ...
  state.config.menuColor = $("cfgMenuColor").value || "#102820";
  ...
  await api("/api/system-config", {
    ...
    menuColor: state.config.menuColor,
    supervisor: state.config.supervisor,
    logo: state.config.logo,
    loginInitials: state.config.loginInitials,
    loginTitle: state.config.loginTitle,
    companyCode: state.config.companyCode,
    loginBackground: state.config.loginBackground,
    loginBrandColor: state.config.loginBrandColor,
    loginButtonColor: state.config.loginButtonColor
  });
  await save();
  toast("Configuracion guardada. El login usara esos datos.");
};
$("cfgLogo").onchange = async () => {
  const file = $("cfgLogo").files[0];
  if (!file) return;
  try {
    state.config.logo = await optimizeLogo(file);
    if (canConfigureSystem()) await api("/api/system-config", { logo: state.config.logo });
    await save();
    toast("Logo guardado y optimizado.");
  } catch {
    toast("No se pudo cargar el logo.");
  }
};
...
async function loadAdminUsers() {
  ...
  if (!data.ok) {
    $("adminUsersList").innerHTML = "<p class='muted'>Solo el dueno puede administrar usuarios.</p>";
    $("adminUserForm").classList.add("hidden");
    return;
  }
  $("adminUserForm").classList.remove("hidden");
  $("adminUsersList").innerHTML = data.users.map(u => `<div class="item"><div class="item-head"><strong>${u.username}</strong><span class="pill">${u.role === "owner" ? "propietario" : u.role === "admin" ? "admin" : "usuario"}</span></div></div>`).join("");
  $("adminCurrentUser").innerHTML = '<option value="">Crear usuario nuevo</option>' + data.users.map(u => `<option value="${u.username}">${u.username}</option>`).join("");
}

$("adminUserForm").onsubmit = async (e) => {
  e.preventDefault();
  const creatingUser = !$("adminCurrentUser").value;
  const data = await api("/api/admin/reset-user", {
    currentUsername: $("adminCurrentUser").value,
    newUsername: $("adminNewUser").value,
    newPassword: $("adminNewPassword").value
  });
  if (data.ok) {
    $("adminUserForm").reset();
    toast(creatingUser ? "Usuario creado por el dueno." : "Usuario actualizado por el dueno.");
    loadAdminUsers();
  } else {
    toast(data.error || "No se pudo actualizar el usuario.");
  }
};
